...
export class MSatuanComponent implements OnInit {
  ...
  getData() {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameter: any, callback) => {
        ...
        this.LandaService.DataGet('/m_satuan/index', params).subscribe((res:any) => {
  ...
  index() {
    ...
    this.pageTitle = 'Data Master Satuan';
    this.getData();
  }
  create() {
    this.empty();
  ...
  delete (val) {
    ...
    Swal.fire({
      ...
      if(result.value) {
        this.LandaService.DataPost('/m_satuan/hapus', data).subscribe((res:any) => {
          if(res.status_code === 200) {
